...
public class ThermalPrinterModule extends ReactContextBaseJavaModule {
  ...
  private static final int MAX_BT_DEVICES = 20;
  private static final String LOG_TAG = "RN_PRINTER_BLE";

  public static final String NAME = "ThermalPrinterModule";
  private Promise jsPromise;
  private BluetoothConnection[] btDevicesList = new BluetoothConnection[MAX_BT_DEVICES];
...
//
  ...
  @ReactMethod
  public void printBluetooth(int index, String payload, boolean autoCut, boolean openCashbox, double mmFeedPaper, double printerDpi, double printerWidthMM, double printerNbrCharactersPerLine, Promise promise) throws JSONException {
    this.jsPromise = promise;
    BluetoothConnection btPrinter = btDevicesList[index];
    if (btPrinter == null) {
      ...
      try {
        ...
        if (pairedDevices.size() > 0) {
          ...
          for (BluetoothDevice device : pairedDevices) {
            btDevicesList[index++] = new BluetoothConnection(device);
